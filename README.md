...
| `POST`    | `/alert`        | ✅             | Create a new price alert for a coin              |
| `GET`     | `/alert/{id}`   | ✅             | Retrieve a specific alert by ID                  |
| `DELETE`  | `/alert/{id}`   | ✅             | Delete a specific alert (owner only)             |
| `WS`      | `/websocket`    | ✅             | Open a persistent WebSocket connection for alerts|
...
│
├── auth/
│   └── auth.py              # Registration, login, JWT encoding/decoding, auth dependency
│
├── database/
...
- **`auth/auth.py`** handles all identity logic, hashing, token creation, and the `get_current_user` dependency injected into protected routes
...
- **`websocket/connections.py`** maintains a registry of open WebSocket connections keyed by user, enabling the Celery worker to push targeted notifications to the right session
...
Alert stored in PostgreSQL (is_active = True)
        │
        ▼
Celery beat worker polls CoinGecko every X seconds
        │
        ▼
...
- Passwords hashed with `pwdlib` plaintext passwords are never stored
- JWT tokens include `sub`, `exp`, and `action` claims; expired or tampered tokens return 401
- `SECRET_KEY` and `DATABASE_URL` loaded from environment variables, the app raises `ValueError` at startup if either is missing
...
# Start PostgreSQL and Redis (required before running)
...
- [ ] Add a `GET /alert` endpoint to return all alerts for the current user
